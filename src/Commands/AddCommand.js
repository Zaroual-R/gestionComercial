...
const AddCommand = () => {
  ...
  const datashow = currentClients ? currentClients.map((item, key) => <LigneAddCommand key={key.id} id={item.idClient} nom={item.nomClient} prenom={item.prenomClient} societe={item.societe} pays={item.pays} ville={item.ville} tel={item.tel} />) : null;
  return (
